...
def LSTModel(trainX, trainY, valX, valY, testX, testY):
    ...
    atPath = os.path.join(os.getcwd(),"MasterThesis","model_LSTM().h5")
    model.save(atPath)
    ...
    loadModelAtPath = os.path.join(os.getcwd(),"MasterThesis","PreTrainedModels","model_LSTM(T.44).h5")
...
def CNNModel(trainX, trainY, valX, valY, testX, testY):
    ...
    atPath = os.path.join(os.getcwd(),"MasterThesis","model_cnn().h5")
    model_cnn.save(atPath)
    
    #following loads the pretrained model to produce reproducilble resluts since models does not produce the same results on every run         
    loadModelAtPath = os.path.join(os.getcwd(),"MasterThesis","PreTrainedModels","model_cnn(0.46).h5")
...
def jointModel(trainX_Text, trainY_Text,  valX_Text, valY_Text, testX_Text, testY_Text, trainX_Aud, trainY_Aud,  valX_Aud, valY_Aud, testX_Aud, testY_Aud):
    ...
    atPath1 = os.path.join(os.getcwd(),"MasterThesis","PreTrainedModels","model_LSTM(T.44).h5")        
    ...
    atPath2 = os.path.join(os.getcwd(),"MasterThesis","PreTrainedModels","model_cnn(0.46).h5")
    ...
    jointModel.fit([trainX_Text,trainX_Aud], trainY_Text, epochs=20, callbacks = [early_stopping], verbose =2,validation_data=([valX_Text,valX_Aud], valY_Text))
    ...
    #save the model at the following path
    atPath = os.path.join(os.getcwd(),"MasterThesis","model_jnn().h5")
    jointModel.save(atPath)
    
    
    print("Load the saved joint neural network model")
    atPath1 = os.path.join(os.getcwd(),"MasterThesis","PreTrainedModels","model_jnn(D.46).h5")
